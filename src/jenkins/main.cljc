(ns jenkins.main
  "Kotodama WASM entrypoint for the Jenkins clean-room actor (L4) — Clojure port.
  ...
  facts (`jenkins.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "jenkins")
(def tier "L4")
...
(def entity-specs
  [{:entity "Build"      :plural "builds"      :id-prefix "jenkins_bui"
    :fields [:number :result :building :duration :timestamp :displayName :description :queueId] :required [:number :result]
    :coerce {:number :int :building :bool :duration :int :timestamp :int :queueId :int} :refs {}}
   {:entity "Job"        :plural "jobs"        :id-prefix "jenkins_job"
    :fields [:name :displayName :description :color :nextBuildNumber :url] :required [:name :displayName]
    :coerce {:nextBuildNumber :int}  :refs {}}
   {:entity "QueueItem"  :plural "queueitems" :id-prefix "jenkins_que"
    :fields [:inQueueSince :why :blocked :buildable :stuck] :required [:inQueueSince]
    :coerce {:inQueueSince :int :blocked :bool :buildable :bool :stuck :bool} :refs {}}
   {:entity "Computer"   :plural "computers"  :id-prefix "jenkins_com"
    :fields [:displayName :name :url :offline :offlineCauseReason :numExecutors] :required [:displayName :name]
    :coerce {:offline :bool :numExecutors :int} :refs {}}
   {:entity "View"       :plural "views"      :id-prefix "jenkins_vie"
    :fields [:name :url :description] :required [:name :url]
    :coerce {} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"jenkins.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "jenkins-compat" :tier tier :entities entities} 200])
...
(defn start! [] :jenkins-compat/ready)
